...
skills/vitaecontext-<module>/wiki/knowledge.md
skills/vitaecontext-<module>/SKILL.md, only if routing changes
...
2. Add `skills/vitaecontext-<module>/SKILL.md`.
3. Add `skills/vitaecontext-<module>/references/` for runtime procedures.
4. Add `skills/vitaecontext-<module>/wiki/index.md` and `wiki/knowledge.md`.
5. Add a `## Wiki context` section to the module `SKILL.md`.
6. Add the module to `src/export-config.json` and provider adapter manifests or wrappers when required.
7. Update `llms.txt`.
8. Regenerate `llms-full.txt`.
9. Update the README modules table.
10. Update `CHANGELOG.md`.
11. Run `npm run validate`.
12. Run an export smoke test when provider packaging changed.
...
5. Diff the official source behavior against `skills/vitaecontext-<module>/wiki/knowledge.md`.
...
- `.agents/plugins/plugins/vitaecontext/skills/` native Codex plugin mirror. Generate it with `npm run sync:codex-plugin`.
- `llms-full.txt`. It is generated from root and module wiki files.

Provider adapter folders under `providers/` are thin wrappers. Put methodology changes in `hub/` and `skills/`, not in provider adapter notes.
...
- `.agents/plugins/plugins/vitaecontext/.codex-plugin/plugin.json` - synchronized by `npm run sync:codex-plugin`.
- Root `gemini-extension.json` and the provider `gemini-extension.json` files.

## Pull request checklist

Before opening a PR that touches knowledge, skills, wiki entries, or sources:

- Read `.assets/docs/architecture-map.md` — covers how hub/, skills/, provider adapters, and MCP relate to each other.
...
- Regenerate and check the native Codex plugin mirror with `npm run sync:codex-plugin` and `npm run check:codex-plugin`.
- Update README and CHANGELOG for user-visible behavior changes.
- Run `npm run validate`.
- Run `node bin/vitaecontext.mjs export --provider all --output /tmp/vitaecontext-export --force` when provider packaging or mirrored files changed.
- Run `npm run validate:package` when package contents changed.

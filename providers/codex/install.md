...
Do not hand-maintain a second Codex skill tree. Author in `skills/`, keep Codex adapter notes in `providers/codex/`, then install a Codex-facing layout when needed. The native plugin mirror under `.agents/plugins/plugins/vitaecontext/skills/` is generated with `npm run sync:codex-plugin`.
